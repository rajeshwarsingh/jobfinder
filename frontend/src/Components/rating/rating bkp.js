...
import { createRating, getRatings, patchRating } from '../../Actions'
...
export class Rating extends React.Component {
	...
	handleRatingSubmit() {
		...
		if (this.state.ratingFlag === 'new') {
			...
			let body = {
				...
				ratingTypeId: this.props.serviceData.serviceId,
	...
	handleEditShow = () => {
		//    alert('called')
		this.setState({ setEditShow: true });
	}
					...
					<ul className="dashboard-box-list">
						{!(this.state.ratingData && this.state.ratingData.ratingId) &&
							<li>
								...
								{/* <a href="#small-dialog-2" className="popup-with-zoom-anim button ripple-effect margin-top-5 margin-bottom-10"><i className="icon-material-outline-thumb-up"></i> View all review</a> */}
								<Button style={{ display: 'inline-block', backgroundColor: '#2a41e8', fontSize: '13px', fontWeight: '600', lineHeight: '20px' }} variant="primary" onClick={this.handleShow}>
									<i className="icon-material-outline-thumb-up"></i> Leave a Review
							</Button>&nbsp;
							<Button style={{ display: 'inline-block', backgroundColor: '#2a41e8', fontSize: '13px', fontWeight: '600', lineHeight: '20px' }} variant="primary" onClick={()=> this.props.history.push(`/allrating/${this.state.ratingData.ratingId}`)}>
									<i className="icon-material-outline-thumb-up"></i> View all review
							</Button>
								...
								<Modal show={this.state.setShow} onHide={this.handleClose}>
									...
									<Modal.Footer>
										...
										<Button style={{ backgroundColor: '#2a41e8' }} variant="primary" onClick={this.handleRatingSubmit}>
											Save Changes
								</Button>
									</Modal.Footer>
								</Modal>
						...
						{this.state.ratingData && this.state.ratingData.ratingId &&
							<li>
								...
								<div><Button style={{ display: 'inline-block', backgroundColor: '#2a41e8', fontSize: '13px', fontWeight: '600', lineHeight: '20px' }} variant="primary" onClick={this.handleEditShow}>
									<i className="icon-material-outline-thumb-up"></i> Edit Review
								</Button>&nbsp;
								<Button style={{ display: 'inline-block', backgroundColor: '#2a41e8', fontSize: '13px', fontWeight: '600', lineHeight: '20px' }} variant="primary" onClick={()=> this.props.history.push(`/allrating/${this.state.ratingData.ratingTypeId}`)}>
									...
									<Modal.Body>
										...
										<form method="post" id="change-review-form">
											<div className="feedback-yes-no">
												<strong>Your Rating</strong>
												<div className="leave-rating">
													<input type="radio" name="rating" id="rating-1" value="1" onClick={this.handleEditShow} checked={("1" === this.state.rating.toString())} />
													<label for="rating-1" className="icon-material-outline-star" ></label>
													<input type="radio" name="rating" id="rating-2" value="2" onClick={this.handleEditShow} checked={("2" === this.state.rating.toString())} />
													<label for="rating-2" className="icon-material-outline-star"></label>
													<input type="radio" name="rating" id="rating-3" value="3" onClick={this.handleEditShow} checked={("3" === this.state.rating.toString())} />
													<label for="rating-3" className="icon-material-outline-star"></label>
													<input type="radio" name="rating" id="rating-4" value="4" onClick={this.handleEditShow} checked={("4" === this.state.rating.toString())} />
													<label for="rating-4" className="icon-material-outline-star"></label>
													<input type="radio" name="rating" id="rating-5" value="5" onClick={this.handleEditShow} checked={("5" === this.state.rating.toString())} />
													<label for="rating-5" className="icon-material-outline-star"></label>
												</div><div className="clearfix"></div>
											</div>
