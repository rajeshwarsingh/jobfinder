...
export class HiremngrActivePost extends React.Component {
  constructor () {
    super()
    this.state = {
      postsData: []
  ...
  render () {
    ...
    const showPosts = this.state.postsData.length ? (this.state.postsData.map((item, i) => {
      return <Post key={i} post={item} {...this.props} />
    })) : (<div><h1>No Post available to show!</h1></div>)
