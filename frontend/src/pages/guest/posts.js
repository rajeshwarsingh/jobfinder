	...
	getPostData() {
		...
		getPosts(filter, (err, res) => {
			...
			countPosts(filter.where,(countErr, count)=>{
				if (res) {
					this.setState({ postsData: Array.isArray(res) ? res : [], isClearFilter:false, postCount:count.count, spinner:false })
				}
			})
		})
	}
	...
	render() {
		...
		const posts = this.state.postsData.length?(this.state.postsData.map((item, i) => {
			...
			<div className="notification error ">
				<p>No {displayNameSettings.post} found!</p>
						...
						<div className="pagination-container margin-top-20 margin-bottom-20">
							<nav className="pagination">
								<ul>
									...
									{(this.state.currentPage*this.state.limit)<this.state.postCount && <li onClick={this.onNextClick} className="pagination-arrow"><a className="ripple-effect"><i className="icon-material-outline-keyboard-arrow-right"></i></a></li>}
								</ul>
							</nav>
						</div>
						<div className="clearfix"></div>
