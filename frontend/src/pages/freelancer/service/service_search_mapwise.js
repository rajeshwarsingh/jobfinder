...
class ServiceSearchItem extends React.Component {
	...
	render() {
		...
		return (
			<div href="#" className="job-listing" onClick={this.handleServiceClick}>
...
export class ServiceSearchMapwise extends React.Component {
	constructor() {
		super()
		this.state = {
			servicesData: [],
		...
		this.handleHiddenServiceValue = this.handleHiddenServiceValue.bind(this)
	...
	getServiceData() {
		...
		getServices(filter, (err, res) => {
			...
			if (res) {
				this.setState({ servicesData: (Array.isArray(res)?res:[]) })
	...
	render() {
		console.log('render********', this.state.servicesData )
		let serviceData = {}
		...
		if(this.state.servicesData && this.state.servicesData.length){
		services = this.state.servicesData.map((item, i) => {
			serviceData[i] = {
				"pagename": `service/${item.serviceId}`,
				"imageUrl": (item.serviceLogo && item.serviceLogo.url) || '',
				"title": item.title || '',
			...
			<div className={`${this.state.enableSidebar}`}>
				...
				<div className="full-page-content-container" data-simplebar>
					<div className="full-page-content-inner">
						...
						{/* <input ref={input => this.inputElement = input} type="hidden" value={serviceData} id="myInput"></input> */}
						<input ref={this.inputRef} value={serviceData} onChange={this.handleHiddenServiceValue} id='myInput' type="hidden" />
						...
						<div className="notify-box margin-top-15">
							<div className="switch-container">
								...
								<span className="switch-text">Search result for your Nearest {displayNameSettings.service}</span>
								{/* </label> */}
							</div>
